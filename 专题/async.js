...
2、await 必须在async中执行
3、遇到await 后面如果是一个异步操作对象，立即返回 promise 对象；
遇到异步操作，然后等待异步操作执行完后，在执行函数体内的后的代码，
...
// 同步执行，顺序输出
// 思路分为2步，同步执行和顺序输出
// 同步执行返回promise 对象，不返回结果
// 顺序输出，此时将所有promise 对象的结果顺序输出
...
// 在一个事件循环中，宏任务执行完毕后，立即执行当前微任务队列中的所有微任务（依次执行）
// 当前微任务执行完毕后，开始执行渲染，然后GUI线程接管渲染
// 渲染完毕后，JS线程继续接管，开始下一个宏任务（从事件队列中获取）
// setImmediate 和 setTimeout 同属于宏任务，但是在同一个事件循环中，setImmediate 比 setTimeout先执行
